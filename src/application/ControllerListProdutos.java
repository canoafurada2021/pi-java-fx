        ...
        txtPesquisa.textProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if (newValue != null && !newValue.isEmpty()) {
                ...
                listaFiltrada.setPredicate(veiculo -> {
                    ...
                    String id = String.valueOf(veiculo.getId_veiculo()).toLowerCase();
                    String marca = veiculo.getMarca().toLowerCase();
                    String ano = String.valueOf(veiculo.getAno()).toLowerCase();
                    String nome = veiculo.getNome();
                    String precoDia = String.valueOf(veiculo.getPreco_por_dia()).toLowerCase();
                    String quant = String.valueOf(veiculo.getUnidade_em_estoque()).toLowerCase();


                    return nome.contains(termoBusca) || id.contains(termoBusca) || marca.contains(termoBusca) || ano.contains(termoBusca) || precoDia.contains(termoBusca) || quant.contains(termoBusca);
				...
				return new TableCell<>() {
					...
					private final HBox buttonContainer = new HBox (viewButton, editButton, vizuButton);
					{
						...
						vizuButton.setOnAction(event -> {
							//passar inform pr TL DE VIZU
							Veiculo veiculo = getTableView().getItems().get(getIndex());
							String idVeic = String.valueOf(veiculo.getId_veiculo());
